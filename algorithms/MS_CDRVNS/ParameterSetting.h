...
{
    printf("Usage: CDRVNS -i <instance_name>  -o <results_filename> -s <seed> -m <archive_size> [-t <max_time>] [-e <nevals_factor>]\n");
...
bool GetParameters(int argc,char * argv[])
{
    ...
    while ((c = GetOption (argc, argv,":h:s:o:i:q:g:b:w:m:t:e:",optarg)) != '\0')
    {
        switch (c)
        {
            case 'h' :
